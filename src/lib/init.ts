// Initialize MongoDB Atlas with default data if empty
export async function initializeMongoDB() {
  try {
    console.log('Initializing MongoDB Atlas data...');
    
    // Check if vehicles exist, if not create default ones
    try {
      const { vehicleAPI } = await import('./api');
      const existingVehicles = await vehicleAPI.getAvailable();
      
      if (!existingVehicles || existingVehicles.length === 0) {
        console.log('Creating default vehicles in MongoDB...');
        ...
        // Create vehicles in MongoDB via admin API
        for (const vehicle of defaultVehicles) {
          try {
            await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/vehicles`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify(vehicle)
            });
          } catch (error) {
            console.log('Vehicle creation failed (may already exist):', vehicle.name);
          }
        }
        
        console.log('Default vehicles created successfully in MongoDB');
      }
    } catch (error) {
      console.log('Error checking vehicles (MongoDB may not be available):', error);
    }
